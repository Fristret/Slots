...
object Server extends IOApp {
  ...
  private[server] def httpApp(topic: Topic[IO, String], ref: Ref[IO, Map[Mail, Player]], cache: Ref[IO, Map[Token, Instant]]) = {
    messageRoute(topic, cache, ref)
  }.orNotFound
  ...
  override def run(args: List[String]): IO[ExitCode] = {
    for {
      ...
      ref <- Ref[IO].of(Map.empty[Mail, Player])
