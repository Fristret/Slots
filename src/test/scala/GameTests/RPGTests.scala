...
class RPGTests extends AnyFreeSpec with Matchers{
  ...
  "enemy attack" - {
    "success damage" in {
      val stage: Stage = Stage(2, 3, Mob(2, 1, 1), Hero(2, 1, Ammunition(0, 2, 0, 0, -1)), 1)
      enemyAttack(stage.enemy, stage.hero) should be (Hero(1, 1, Ammunition(0, 2, 0, 0, -1)))
    }

    "evade damage" in {
      val stage: Stage = Stage(2, 3, Mob(2, 1, 1), Hero(2, 1, Ammunition(0, 2, 0, 0, 10)), 1)
      enemyAttack(stage.enemy, stage.hero) should be (Hero(2, 1, Ammunition(0, 2, 0, 0, 10)))
    }

    "shield block" in {
      val stage: Stage = Stage(2, 3, Mob(2, 1, 1), Hero(2, 1, Ammunition(0, 2, 0, 1, 0)), 1)
  ...
  "hero attack" - {
    "success damage" in {
      val stage: Stage = Stage(2, 3, Mob(2, 1, 0), Hero(2, 1, Ammunition(0, 2, 0, 0, 0)), 1)
      heroAttack(stage.enemy, stage.hero) should be (Mob(-1, 1, 0))
    }
    "success evade" in {
      val stage: Stage = Stage(2, 3, Mob(2, 1, 10), Hero(2, 1, Ammunition(0, 2, 0, 0, 0)), 1)
      heroAttack(stage.enemy, stage.hero) should be (Mob(2, 1, 10))
    }
  }
  ...
  "fight" - {
    "No dmg" in {
      val stage: Stage = Stage(2, 3, Mob(2, 1, 0), Hero(2, 1, Ammunition(0, 2, 0, 0, 0)), 1)
      fight(NotDoDmgOrDo = true, stage, Bet(20)) should be (Map(Stage(2, 3, Mob(2, 1, 0), Hero(2, 1, Ammunition(0, 2, 0, 0, 0)), 2) -> 0))
    }
    "Kill enemies" in {
      val stage1: Stage = Stage(1, 1, Mob(1, 1, 0), Hero(2, 1, Ammunition(0, 0, 0, 0, 0)), 1)
